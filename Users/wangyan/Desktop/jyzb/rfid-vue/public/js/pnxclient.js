...
var JIT_GW_ExtInterface = (function () {
  ...
  var addActioveX = function () {
    ...
    // 设置 classid 属性
    obj.setAttribute("classid", "clsid:9DD991F7-6FB0-4004-95A4-0A55006A8C42");
    // 设置宽度为 0
    obj.setAttribute("width", "0");
    // 设置高度为 0
    obj.setAttribute("height", "0");
    // 设置 id 为 PNXGWClient
    obj.setAttribute("id", "PNXGWClient");
    // 将 object 元素添加到 body 中
    document.body.appendChild(obj);
  };
  var addNPPlugin = function () {
    // document.write(
    //   "<embed type='application/x-jit-auth-plugin' id='PNXGWClient' width='0' height='0'></embed>"
    // );
    // 创建一个 object 元素
    var obj = document.createElement("embed");
    // 设置 classid 属性
    obj.setAttribute("type", "application/x-jit-auth-plugin");
    // 设置宽度为 0
    obj.setAttribute("width", "0");
    // 设置高度为 0
    obj.setAttribute("height", "0");
  ...
  return {
    ...
    GetSignVersion: function () {
      if (useActioveX || useNPPlugin) {
        ...
        var jsonstr = "";
        var result = JSON.parse(
          SendAndWaitMessageEx("GetSignVersion", jsonstr)
        );
        return result.value;
      }
    },
    ...
    CallQueueClear: function () {
      if (useActioveX || useNPPlugin) {
        ...
        var jsonstr = "";
        var result = JSON.parse(
          SendAndWaitMessageEx("CallQueueClear", jsonstr)
        );
        return result.value;
      }
    },
    ...
    DoClientSecurityCheck: function (
      strGatewayAddress,
      usGatewayPort,
      strSecurityPolicys
    ) {
    ...
    GetPinCode: function () {
      if (useActioveX || useNPPlugin) {
        ...
        var jsonstr = "";
        var result = JSON.parse(
          SendAndWaitMessageEx("GetPinCode", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    SetForcePinDialog: function (isForcePinDialog) {
      if (useActioveX || useNPPlugin) {
        ...
        var result = JSON.parse(
          SendAndWaitMessageEx("SetForcePinDialog", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    MessageAuth: function (bstrGatewayAddress, bstrAttributeName, bstrAppID) {
      if (useActioveX || useNPPlugin) {
        return PNXGWClient.MessageAuth(
          bstrGatewayAddress,
          bstrAttributeName,
          bstrAppID
        );
        ...
        var jsonstr = {
          bstrGatewayAddress: bstrGatewayAddress,
          bstrAttributeName: bstrAttributeName,
          bstrAppID: bstrAppID,
        };
        var result = JSON.parse(
          SendAndWaitMessageEx("MessageAuth", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    GetPhoto: function (bstrSvrIP, usSvrPort, bstrID) {
      if (useActioveX || useNPPlugin) {
        ...
        var result = JSON.parse(
          SendAndWaitMessageEx("GetPhoto", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    GetCertList: function () {
      if (useActioveX || useNPPlugin) {
        ...
        var jsonstr = "";
        var result = JSON.parse(
          SendAndWaitMessageEx("GetCertList", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    P1SignWithPin: function (bstrSN, bstrPIN, bstrOrg) {
      if (useActioveX || useNPPlugin) {
        ...
        var result = JSON.parse(
          SendAndWaitMessageEx("P1SignWithPin", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
    ...
    SetSessionToken: function (strGatewayAddress, strToken) {
      if (useActioveX || useNPPlugin) {
        ...
        var result = JSON.parse(
          SendAndWaitMessageEx("SetSessionToken", JSON.stringify(jsonstr))
        );
        return result.value;
      }
    },
