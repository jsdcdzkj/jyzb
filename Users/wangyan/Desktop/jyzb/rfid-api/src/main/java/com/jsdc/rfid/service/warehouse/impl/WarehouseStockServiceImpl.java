...
public class WarehouseStockServiceImpl extends ServiceImpl<WarehouseStockMapper, WarehousingStock> implements WarehouseStockService {
    ...
    @Override
    public PageInfo<WarehousingStock> pageQuery(WarehousingStockDto warehousingStockDto) {
        ...
        if(null != warehousingStockDto.getDeptIds() && warehousingStockDto.getDeptIds().size() > 0){
            warehousingStockDto.setDeptIds(warehousingStockDto.getDeptIds());
        }else{
            warehousingStockDto.setDeptIds(deptIds);
        }


        PageHelper.startPage(warehousingStockDto.getPage(), warehousingStockDto.getLimit());
...
//        }
    ...
    public void deliveryStock(WarehousingDelivery warehousingDelivery) {
        ...
        //批次
        String batchNo = DateUtil.format(new Date(), "yyyy-MM-dd HH:mm:ss.SSS");
            ...
            switch (warehousingDelivery.getDelivery_type()) {
                ...
                case 2:
                    ...
                    stockDetails.forEach(s -> {
                        ...
                        stockDetail1.setBatch_no(batchNo);
                        stockDetail1.setEquip_out_num(0);
                        stockDetail1.setEquip_in_num(s.getEquip_out_num());
                        stockDetail1.setStock_type(warehousingDelivery.getDelivery_type());
                        stockDetail1.setEquip_status(warehousingDelivery.getEquip_status());
                        stockDetail1.setDept_id(null);
                        stockDetail1.setUse_dept(null);
    ...
    public WarehouseStockDetailVo detail(WarehousingStockDto warehousingStockDto) {
        ...
        detailsMap.forEach((key, sDetails) -> {
            sDetails.forEach(s -> {
                int inNum = s.getEquip_in_num() == null ? 0 : s.getEquip_in_num();
                int outNum = s.getEquip_out_num() == null ? 0 : s.getEquip_out_num();
                int equipNum = Math.abs(inNum - outNum);
                s.setEquip_num(equipNum);
                s.setPrice(s.getUnit_price().multiply(new BigDecimal(equipNum)).abs());
