...
public class CascadeSelectTool {
    ...
    public CascadeSelectTool createHead(List<String> heads) {
        ...
        for (int i = 0; i < heads.size(); i++) {
            ...
            XSSFCell startCell1 = startRow.createCell(i);
            startCell1.setCellValue(value);
            startCell1.setCellStyle(cellStyle);
            this.mainSheet.autoSizeColumn(i);
            this.mainSheet.setColumnWidth(i, this.mainSheet.getColumnWidth(i) * 3);
        }
        return this;
    }
    public CascadeSelectTool createEmptyList(int row ,int col) {
        XSSFCellStyle cellStyle = this.workbook.createCellStyle();
        cellStyle.setBorderBottom(BorderStyle.THIN);
        cellStyle.setBorderLeft(BorderStyle.THIN);
        cellStyle.setBorderRight(BorderStyle.THIN);
        ...
        row = row + 1;
        for (int i = 1; i < row; i++) {
            XSSFRow startRow = this.mainSheet.createRow(i);
            startRow.setHeightInPoints(18);
            for (int j = 0; j < col; j++) {
                XSSFCell startCell1 = startRow.createCell(j);
                startCell1.setCellStyle(cellStyle);
            }
        }
        return this;
     ...
     * 设置二级级联下拉框数据
     */
    public CascadeSelectTool setCascadeDropDownBox(List<Integer> selectColList, Integer startColumn, Integer endColumn, Map<String, List<String>> dataMap) {
        // 获取所有sheet页个数
        ...
        if (dataMap != null && !dataMap.isEmpty()) {
            ...
            if (firstRow != null) {
                hiddenSheet.removeRow(firstRow);  // 只有当第0行存在时才删除
            }

            // 填充数据到隐藏sheet页
            int startRowNum = hiddenSheet.getLastRowNum() + 1;
            for (String key : dataMap.keySet()) {
                XSSFRow row = hiddenSheet.createRow(startRowNum++);
                row.createCell(0).setCellValue(key);
                List<String> sons = dataMap.get(key);
                for (int i = 0; i < sons.size(); i++) {
                    row.createCell(i + 1).setCellValue(sons.get(i));
                }
                ...
                name.setRefersToFormula(hiddenSheetName + "!" + range);
            }
        ...
        // 设置级联下拉框
        String mainFormula = hiddenSheetName + "!$A$" + startColumn + ":$A$" + endColumn;  // 主级下拉框
        for (int i = 0; i < selectColList.size(); i++) {
            ...
            if (i == 0) {
                ...
                Integer parentCol = selectColList.get(i - 1);
                // 设置状态列的级联下拉框，依赖装备类型列的选择
                String indirectFormula = "INDIRECT($" + decimalToTwentyHex(parentCol + 1) + "$" + (firstRow + 1) + ")";
                indirectFormula = "INDIRECT($" + decimalToTwentyHex(parentCol + 1) + (firstRow + 1) + ")";
