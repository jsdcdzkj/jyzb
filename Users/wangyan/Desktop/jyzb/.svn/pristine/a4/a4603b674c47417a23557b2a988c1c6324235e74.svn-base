...
//        }
    ...
    public JSONArray getTree(AssetsType assetsType) {
        ...
        sortedAssetsTypes.forEach(x -> {
            ...
            item.put("shelf_life", x.getShelf_life());
            item.put("measurement_unit", x.getMeasurement_unit());
            item.put("remarks", x.getRemarks());
            item.put("is_statistics", x.getIs_statistics());
            item.put("is_disable", x.getIs_disable());
    ...
    // 服务层实现（兼容Java 8）
    public JSONArray getFullTreeData() {
        List<AssetsType> allData = selectList(
                new QueryWrapper<AssetsType>()
                        .eq("is_del", "0")
                        .orderByAsc("level", "id")
        );

        // 构建ID映射表
        Map<Integer, AssetsType> dataMap = new HashMap<>();
        for (AssetsType item : allData) {
        ...
        for (AssetsType item : allData) {
            ...
            // Java 8兼容缩进实现↓↓↓
            int indentSize = item.getLevel()  != null ? item.getLevel()  * 4 : 0;
            node.put("treeTitle",  generateIndent(indentSize) + item.getAssets_type_name());
            node.put("code",  item.getAssets_type_code());
            node.put("remarks",  item.getRemarks());
            node.put("level",  item.getLevel());
            node.put("fullPath",  buildFullPath(item, dataMap));
            result.add(node);
        }
        return result;
    }
    ...
    // 构建全路径方法
    private String buildFullPath(AssetsType node, Map<Integer, AssetsType> dataMap) {
        LinkedList<String> path = new LinkedList<>();
        while (node != null) {
            path.addFirst(node.getAssets_type_name());
            Integer parentId = node.getParent_id();
            node = (parentId != null && parentId > 0) ? dataMap.get(parentId)  : null;
        }
