  ...
  <el-drawer :visible.sync="dialogFormVisible" :size="variables.drawWidth" @close="close" modal-append-to-body>
    ...
    <div style="padding: 16px;">
      <el-table :data="detailInfo.enterDetails" stripe border fit height="calc(100vh - 290px)" highlight-current-row>
        ...
        <el-table-column label="装备类型" width="200" prop="equip_type_name" align="center"
        ...
        <el-table-column label="计量单位" width="150" prop="measurement_unit" align="center"></el-table-column>
        ...
        <el-table-column label="生产日期" prop="produce_date" align="center" show-overflow-tooltip>
          <template #default="{ row }">
            <el-date-picker size="mini" style="width: 100%;" placeholder="请选择日期" v-model="row.produce_date" type="date"
              value-format="yyyy-MM-dd"></el-date-picker>
          </template>
        </el-table-column>
        <el-table-column label="保质年限(月)" prop="use_year" align="center">
          <template #default="{ row }">
            <el-input v-model="row.use_year" type="number" placeholder="请输入" clearable></el-input>
          </template>
        </el-table-column>
...
import { addInOrder, editInOrder, inOrderDetail } from '@/api/inventory'
export default {
  name: 'detail',
  data() {
    return {
  ...
  methods: {
    save(row) {
      ...
      if (!row.use_year) {
        this.$message.error('请输入保质年限');
        return;
      }
      if (this.loading) return;
      this.loading = true;
      editInOrder(row).then(() => {
