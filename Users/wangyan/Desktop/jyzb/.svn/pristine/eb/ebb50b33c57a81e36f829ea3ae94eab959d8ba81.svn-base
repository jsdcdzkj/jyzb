  ...
  <div class="app-container">
    <div class="filter_container">
      ...
      <div class="filter_box">
        <el-form :inline="true" class="demo-form-inline topsearch" @submit.native.prevent>
          ...
          <el-form-item>
            <el-select clearable v-model="listQuery.dateType" placeholder="装备状态" style="width: 100px!important;">
              <el-option label="正常装备" :value="3"></el-option>
              <el-option label="过期装备" :value="1"></el-option>
              <el-option label="临期装备" :value="2"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-select clearable v-model="listQuery.equip_status" style="width: 100px!important;" @change="initStatus">
              <el-option label="全部装备" value=""></el-option>
              <el-option label="库存装备" :value="0"></el-option>
              <el-option label="在用装备" :value="1"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="listQuery.equip_status === 0">
            <el-select clearable v-model="listQuery.warehouseIds" placeholder="选择仓库" multiple key="warehouse">
              <el-option v-for="el, index in warehouseList" :key="index" :label="el.warehouse_name"
                :value="el.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="listQuery.equip_status === 1">
            <el-cascader v-model="listQuery.deptIds" :options="deptList" :show-all-levels="false"
              :props="{ checkStrictly: true, value: 'id', label: 'title', emitPath: false, multiple: true }" clearable
              placeholder="选择机构"></el-cascader>
          </el-form-item>
          <el-form-item>
            <el-select clearable v-model="listQuery.equip_type" placeholder="装备类型">
          ...
          <el-form-item style="float: right;">
            <el-button size="small" type="success" @click="handleMonth">月度转结</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
    <div class="content-box">
      ...
      <div class="el-table-box">
        <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" stripe border fit height="100%"
          ...
          <el-table-column label="所属对象" prop="homeObjectDesc" align="center" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column label="出厂日期" width="100" prop="produce_date_format" align="center"></el-table-column>
          <el-table-column label="保质期(月)" width="100" prop="use_year" align="center"></el-table-column>
          <el-table-column label="过期时间" width="100" prop="expired_date_format" align="center"></el-table-column>
          <el-table-column label="入库时间" width="100" prop="enter_time_format" align="center"></el-table-column>
    ...
    <el-dialog title="库存详情" :visible.sync="showDialog" width="1200px">
      ...
      <el-table :data="detailInfo.stockDetails" ref="detailTable" stripe border fit height="auto" max-height="60vh"
        ...
        <el-table-column align="center" label="出入库类型" width="100">
          <template #default="{ row }">
            <span v-if="row.stock_type == '0'">手动入库</span>
            <span v-else>
              {{ row.stock_type == 1 ? '调拨' : row.stock_type == 2 ? '领用' : '处置' }}
            </span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="equip_num" label="数量" width="60"></el-table-column>
        <el-table-column align="center" prop="measurement_unit" label="计量单位" width="80"></el-table-column>
        <el-table-column align="center" prop="price" label="价值"></el-table-column>
        <el-table-column align="center" label="操作" width="130">
          <template #default="{ row }">
            <el-button type="primary" size="mini" @click="handChangeNum(row)">
              编辑
            </el-button>
...
import { getStockList, getEquipmentStock, updateStock } from '@/api/inventory'
  ...
  methods: {
    ...
    handChangeNum(row) {
      this.$prompt('请输入数量', '提示', {
        ...
        inputValue: Math.abs(row.equip_num),
        inputPattern: /^\d+(\.\d+)?$/,
        inputErrorMessage: '请输入正确的数量'
      }).then(({ value }) => {
        if (row.equip_num < 0 && value > Number(this.detailInfo.equipNum) + Math.abs(row.equip_num)) {
          this.$message({
            type: 'error',
            message: '数量不能大于当前数量'
          });
        ...
        updateStock({
          ...
          this.handleView(this.detailRow);
          this.fetchData();
        })
      })
    },
    handleDelete(id) {
      this.$confirm('确定删除吗？', '提示', {
        ...
        updateStock({
          ...
          if (this.detailInfo.stockDetails.length == 1) {
