  ...
  <el-drawer :visible.sync="dialogFormVisible" :size="variables.drawWidth" @close="close" :wrapperClosable="false">
    ...
    <div class="box-card">
      ...
      <el-form ref="baseInfo" :model="formInline" :rules="rules" style="padding: 10px 20px;">
        <el-row :gutter="20" type="flex">
          ...
          <el-col v-if="formInline.equip_status !== ''">
            ...
            <el-form-item prop="use_dept" v-if="formInline.equip_status === 1" key="dept">
              <el-cascader v-model="formInline.use_dept" style="width: 100%;" disabled :options="deptList"
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-input type="textarea" placeholder="备注" resize="none" :rows="3"
              v-model="formInline.description"></el-input>
          </el-col>
          <el-col :span="12">
            <el-upload ref="uploadFile" :limit="1" :action="uploadUrl" :on-error="handleError"
              :before-upload="handleBeforeUpload" :on-success="handleSuccess" :on-exceed="onExceed"
              :on-remove="onRemove" :headers="{ 'accesstoken': token }">
      ...
      <div style="padding: 16px;">
        <el-table :data="formInline.deliveryDetails" ref="tableBox" stripe border fit height="calc(100vh - 410px)">
          ...
          <el-table-column label="装备类型" align="center">
            <template #default="{ row, $index }">
              <el-select size="mini" v-model="row.equip_type" placeholder="装备类型" @change="changeType($event, $index)">
                <el-option v-for="el, index in assetsType" :key="index" :label="el.label" :value="el.value"></el-option>
              </el-select>
            </template>
          </el-table-column>
          ...
          <el-table-column label="型号" align="center">
            <template #default="{ row, $index }">
              <el-select size="mini" v-model="row.equip_model" placeholder="装备型号" @change="changeModel($event, $index)"
                v-if="row.equip_name">
                <el-option v-for="el, index in row.modelList" :key="index" :label="el.label || '空'"
                  :value="el.value"></el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="出厂日期" width="155" align="center">
            <template #default="{ row, $index }">
              <el-select size="mini" v-model="row.produce_date" placeholder="出厂日期"
                v-if="row.equip_model || row.equip_name" @change="changeDate($event, $index)">
...
export default {
  ...
  methods: {
    ...
    changeName(val, index) {
      const item = this.formInline.deliveryDetails[index];
      item.produce_date = '';
      item.stockCount = '';
      item.modelList = item.nameList.find(el => el.value == val).child || [];
      item.equip_model = item.modelList[0].value;
      item.dateList = item.modelList.length ? item.modelList[0].child || [] : [];
    },
    changeModel(val, index) {
      this.formInline.deliveryDetails[index].produce_date = '';
      this.formInline.deliveryDetails[index].stockCount = '';
      ...
      this.$refs.baseInfo.validate((valid) => {
        if (valid) {
          ...
          const isVerify = list.every(el => {
            if (!el.equip_type || !el.equip_name || !el.produce_date) {
