...
//        }
        ...
        if(!CollectionUtils.isEmpty(bean.getEnterDetails())){
            List<WarehousingEnterDetail> details = bean.getEnterDetails().stream().map(d->{
                ...
                //删除共享流程
                UpdateWrapper<WarehousingBorrow> borrowUpdateWrapper = new UpdateWrapper<>();
                borrowUpdateWrapper.eq(d.getEquip_status() != null && d.getEquip_status() == 0 && d.getWarehouse_id() != null, "warehouse_id", d.getWarehouse_id());
                borrowUpdateWrapper.eq(d.getEquip_status() != null && d.getEquip_status() == 1 && d.getUse_dept() != null, "use_dept", d.getUse_dept());
                borrowUpdateWrapper.set("is_del", "1");
                WarehousingBorrow warehousingBorrow = new WarehousingBorrow();
                warehousingBorrow.update(borrowUpdateWrapper);
    ...
    public ResultInfo toImport(Integer fileId, String is_save){
        ...
        //类型 保留最后一个值
        List<AssetsType> assetsTypeList = this.assetsTypeService.selectList(Wrappers.<AssetsType>lambdaQuery().eq(AssetsType::getIs_del, G.ISDEL_NO).orderByDesc(AssetsType::getCreate_time));
        ...
        //仓库 保留最后一个值
        List<Warehouse> warehouseList = warehouseService.selectList(Wrappers.<Warehouse>lambdaQuery().eq(Warehouse::getIs_del, G.ISDEL_NO).orderByDesc(Warehouse::getCreate_time));
        ...
        //部门 保留最后一个值
        List<SysDepartment> departmentList = sysDepartmentService.selectList(Wrappers.<SysDepartment>lambdaQuery().eq(SysDepartment::getIs_del, G.ISDEL_NO).orderByDesc(SysDepartment::getCreate_time));
...
//        enterQueryWrapper.eq("date_import",DateUtils.formatDate(new Date(), "yyyy-MM"));
            ...
            stockDetailUpdateWrapper.eq("create_user",sysUser.getId());
//        stockDetailUpdateWrapper.eq("date_import",DateUtils.formatDate(new Date(), "yyyy-MM"));
            ...
            for (Map<String, Object> map : readAll) {
                ...
                //装备类型	装备名称	型号	数量	保值期限（月）	出厂日期	状态	归属对象
...
//                }
                ...
                String use_year = null == map.get("保值期限（月）") ? "" : map.get("保值期限（月）").toString();
                if (StringUtils.isBlank(use_year)) {
                    errList.add("保值期限（月）不能为空 ");
                    map.put("保值期限（月）tip", "保值期限（月）不能为空 ");
                } else {
                    if(!StringUtils.isNumeric(use_year)){
                        errList.add("保值期限（月）的数据格式不正确 ");
                        map.put("保值期限（月）tip", "保值期限（月）的数据格式不正确 ");
                    }else{
                        if(Integer.parseInt(use_year) > 999){
                            errList.add("保值期限（月）的数据格式不正确 ");
                            map.put("保值期限（月）tip", "保值期限（月）的数据格式不正确 ");
                        }
